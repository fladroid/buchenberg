#!/bin/bash
# run_kaskada12.sh — dvodijelna kaskada s tvrdim plafonom (s172).
# OGLEDALO kaskade 11: isti oblik, obrnute uloge modela.
#
# root (mistral-large-3:675b @ 0.1)
#   -> PETLJA: ponavljaj KRUG { faza 27 (qwen@0.8 base) -> faza 12 (mistral@0.8 base) }
#      STOP: (a) obje faze kruga prebacile nulu, ili (b) dosegnut MAX_KRUGOVA=3.
#   -> SEED BLOK (uvijek, bez obzira ZASTO se izaslo iz petlje):
#      faza 28 (qwen@0.8 refine-strict) -> faza 24 (mistral@0.8 refine-strict)
...
if [[ -z "$KNJIGA" || -z "$JEZICI" || -z "$OD" || -z "$DO" ]]; then
    echo "Upotreba: bash run_kaskada12.sh --knjiga ID --jezici 'de hr' --od N --do M [--prag 0.95] [--max 3]"
    exit 1
fi
