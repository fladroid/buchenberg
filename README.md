...
| `src/health_check.py` | Infrastrukturna provjera svih komponenti; čita bb bazu. Poziv: `venv/bin/python src/health_check.py` (putanja je i u §12; s173 promašena jer README nije pročitan do kraja) **s175: + sekcija 3b Ollama Cloud potrošnja** (session+sedmicno, uvezeno iz `bb_ollama_usage.py`, snapshot bez delte) |
...
| `src/bb_rupe_pobjednika.py` + `run_rupe.sh` | **s175** — kompaktni intervali (od-do) gdje pobjednik postoji/ne postoji, po (knjiga, jezik) paru. Domen = `bb_recenice` za tu knjigu; pokriva samo parove koji imaju red u `bb_prev_knjige`. `--knjiga ID` `--jezik KOD` (oba opciona, default sve) |
| `src/bb_prazno_svuda.py` + `run_prazno.sh` | **s175** — isto, ali agregirano preko SKUPA jezika: "prazno" = nijedan jezik iz skupa nema pobjednika na toj poziciji, "dotaknuto" = bar jedan ima. `--knjiga ID` `--jezik KOD1 KOD2 ...` (lista, razmakom, bez navodnika; default svi registrovani jezici) |
| `src/bb_ollama_usage.py` + `run_ousage.sh` | **s175** — Ollama Cloud potrošnja (session + sedmično) preko nezvaničnog `/api/usage` endpointa (isti `OLLAMA_API_KEY`, ne login). Čuva stanje u `schedulogs/ollama_stanje.json`, računa deltu od poslednje provjere (agregatno i po modelu). `./run_ousage.sh` (pun izvještaj) ili `--kratko` (jedna linija). NAPOMENA: endpoint nezvaničan (GitHub issues ollama/ollama #15132/#15663/#16448 svi otvoreni) — može se promijeniti bez najave |
| `run_health_delta.sh` | **s175** — satni monitoring skript, pozvan preko Claude Cowork "Scheduled task" (hourly). Pokreće `health_check.py` + `run_ousage.sh --kratko`, poredi broj rupa sa `schedulogs/rupa_stanje.txt`, ispisuje kratku liniju (za push poruku) + piše detaljan blok u `schedulogs/health_check.log`. Cowork prompt ostaje fiksan skelet — sva logika je u ovoj skripti |
